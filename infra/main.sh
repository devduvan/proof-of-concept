...
case $command in
  ...
  "deploy")
    ...
    do
      if  [[ "$env" == "dev" ]]
      then
        fullCommand="cdklocal deploy $i-$env -c env=$env"
      else
  ...
  "deployStack")
    if  [[ "$env" == "dev" ]]
    then
      fullCommand="cdklocal deploy $stack-$env -c env=$env"
    else
  ...
  "destroy")
    if  [[ "$env" == "dev" ]]
    then
      fullCommand="cdklocal destroy --all -c env=$env"
    else
  ...
  "destroyStack")
    if  [[ "$env" == "dev" ]]
    then
      fullCommand="cdklocal destroy $stack-$env -c env=$env"
    else
